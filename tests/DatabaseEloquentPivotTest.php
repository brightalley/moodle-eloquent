...
class DatabaseEloquentPivotTest extends TestCase
{
    ...
    {
        $parent = m::mock('local_eloquent\eloquent\model[getConnectionName,getDates]');
        $parent->shouldReceive('getConnectionName')->andReturn('connection');
        $parent->shouldReceive('getDates')->andReturn([]);
        $pivot = new DatabaseEloquentPivotTestDateStub($parent, ['foo' => 'bar', 'timecreated' => 'foo'], 'table');
