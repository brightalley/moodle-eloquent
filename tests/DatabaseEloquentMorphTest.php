...
class DatabaseEloquentMorphTest extends TestCase
{
    ...
    public function testCreateFunctionOnMorph()
    {
        ...
        $relation = $this->getOneRelation();
        $created = m::mock('local_eloquent\eloquent\model');
    ...
    {
        $relation = $this->getOneRelation();
        $relation->getQuery()->shouldReceive('find')->once()->with('foo', ['*'])->andReturn($model = m::mock('local_eloquent\eloquent\model'));
    ...
    public function testFindOrNewMethodReturnsNewModelWithMorphKeysSet()
    {
        ...
        $relation->getRelated()->shouldReceive('newInstance')->once()->with()->andReturn($model = m::mock('local_eloquent\eloquent\model'));
    ...
    public function testFirstOrNewMethodFindsFirstModel()
    {
        ...
        $relation->getQuery()->shouldReceive('first')->once()->with()->andReturn($model = m::mock('local_eloquent\eloquent\model'));
        $relation->getRelated()->shouldReceive('newInstance')->never();
        $model->shouldReceive('setAttribute')->never();
        $model->shouldReceive('save')->never();
    ...
    public function testFirstOrNewMethodReturnsNewModelWithMorphKeysSet()
    {
        ...
        $relation->getRelated()->shouldReceive('newInstance')->once()->with(['foo'])->andReturn($model = m::mock('local_eloquent\eloquent\model'));
        $model->shouldReceive('setAttribute')->once()->with('morph_id', 1);
        $model->shouldReceive('setAttribute')->once()->with('morph_type', get_class($relation->getParent()));
        $model->shouldReceive('save')->never();
...
class EloquentMorphResetModelStub extends \local_eloquent\eloquent\model
{
}
