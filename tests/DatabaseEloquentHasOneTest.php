...
class DatabaseEloquentHasOneTest extends TestCase
{
    ...
    public function testSaveMethodSetsForeignKeyOnModel()
    {
        ...
        $mockModel = $this->getMockBuilder('local_eloquent\eloquent\model')->setMethods(['save'])->getMock();
    ...
    {
        $relation = $this->getRelation();
        $created = $this->getMockBuilder('local_eloquent\eloquent\model')->setMethods(['save', 'getKey', 'setAttribute'])->getMock();
    ...
    public function testUpdateMethodUpdatesModelsWithTimestamps()
    {
        ...
        $relation->getQuery()->shouldReceive('update')->once()->with(['foo' => 'bar', 'timeupdated' => 100])->andReturn('results');
    ...
    protected function getRelation()
    {
        ...
        $this->related = m::mock('local_eloquent\eloquent\model');
        $this->builder->shouldReceive('getModel')->andReturn($this->related);
        $this->parent = m::mock('local_eloquent\eloquent\model');
        $this->parent->shouldReceive('getAttribute')->with('id')->andReturn(1);
        $this->parent->shouldReceive('getCreatedAtColumn')->andReturn('timecreated');
        $this->parent->shouldReceive('getUpdatedAtColumn')->andReturn('timeupdated');
...
class EloquentHasOneModelStub extends \local_eloquent\eloquent\model
{
